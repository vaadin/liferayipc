...
public class SessionReceiverUI extends SessionAwarePortletUI {
    ...
    private void initializeIPC() {
        ...
        // received data
        liferayIPC.addLiferayIPCEventListener("newPersonInSession", event -> {
            ...
            try {
                ...
                Notification.show(
                        "Received event but failed to retrieve person from the portlet session (I/O exception)");
                return;
            } catch (ClassNotFoundException e2) {
                Notification.show(
                        "Received event but failed to retrieve person from the portlet session (class not found)");
                return;
            }

            if (null == person) {
                Notification.show(
                        "Received event but person not found in shared session");
                return;
            }

            // add a new user to the table based on the message
            // contents
            receiverView.addUser(person.getFirstName(), person.getLastName(),
                    person.getAge());
        });
